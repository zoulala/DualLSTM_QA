...
def main2(_):
    ...
    model = CharLSTM(converter.vocab_size, test=True,
    ...
    arr = model.test(FLAGS.max_length, start, converter.vocab_size)
...
def main(_):
    ...
    model = DualLSTM(converter.vocab_size, test=True,
    ...
    response_matul_state = model.test5_matul(all_samples)  # 有454000个样本，花费时间很长
    while True:
        query = input('query:')
        input_arr,input_len = converter.Q_to_arr(query,FLAGS.num_steps)
        indexs = model.test4(input_arr,input_len, response_matul_state)
